...
# Минимальный набор модулей: только то что нужно для загрузки multiboot ядра с ISO
GRUB_MODULES="biosdisk iso9660 multiboot normal configfile"

# Создаём минимальный образ GRUB
grub-mkimage \
    -O i386-pc-eltorito \
...
# Собираем ISO через xorriso (без grub-mkrescue)
xorriso -as mkisofs \
    -R -J \
