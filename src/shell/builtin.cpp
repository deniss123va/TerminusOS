...
void cmd_cd(char* name) {
    ...
    // ✅ ИСПРАВЛЕНО: Ищем запись (0x10 = директория)
    ...
    if (strcmp(name, "..") == 0) {
        ...
        // Обновляем путь (убираем последний сегмент)
        dirname(current_path);
        
        // Устанавливаем кластер родителя
        // ВАЖНО: если target_cluster == 0, это означает корневую директорию
        if (target_cluster == 0) {
            current_dir_cluster = FAT_ROOT_DIR_CLUSTER;
        } else {
...
void cmd_create(char* name) {
    ...
    // Create empty file
    char empty_content[1] = {0};
    bool success = fat_create_file(name, empty_content, 0);
